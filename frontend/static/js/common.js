...
(function(){
  ...
  if (cached) { const el = document.getElementById('shopLogo'); if (el) el.innerHTML = esc(cached) + ' <span id="navVersion">(v1.8)</span>'; }
})();
fetch('/api/settings')
  .then(r => r.json())
  .then(s => {
    if (s.shop_name) {
      ...
      if (el) el.innerHTML = esc(s.shop_name) + ' <span id="navVersion">(v1.8)</span>';
