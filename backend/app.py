...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5001)
        ...
        for page in reader.pages:
            ...
            if text.strip():
                pages.append(text)
            # Vendor/order PDFs commonly expose each table row as:
            # Code Item Qty TP Disc/Bonus/Tax Net.  Convert only the columns
            # Demand Search needs into its native "Item (qty)" text format.
